...
with open('filenames.txt') as f:
    EDIT_FILES = [x.strip() for x in f.readlines()]
...
def copy_files(submission_dir, out_dir, join_out):
    ...
    for cur_dir in student_dirs:
        ...
        for cur_edit_file in EDIT_FILES:
            if(os.path.exists(os.path.join(full_dir, cur_edit_file))==False):
                with open(os.path.join(full_dir, cur_edit_file),'w') as f:
                    pass
